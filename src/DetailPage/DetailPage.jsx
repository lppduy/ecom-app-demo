  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch(
        `https://firebasestorage.googleapis.com/v0/b/funix-subtitle.appspot.com/o/Boutique_products.json${query}&alt=media&token=dc67a5ea-e3e0-479e-9eaf-5e01bcd09c74`,
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const selectedData = slicedData.find(item => item._id.$oid === productId);
          ...
          {review === 'description' ? (
            <div className="tab-pane fade show active">
              <div className="pt-4 pb-4 bg-white">
                ...
                <br></br>
                <p className="text-muted text-small mb-0" style={{ whiteSpace: 'pre-wrap' }}>
                  {detail.long_desc}
                </p>
              </div>
            </div>
